...
func init_travelers() -> void:
	...
	for direction in routes:
		...
		match direction:
			"down to up":
				route = routes[direction].pop_front()
				var array = []
				
				if route.front().grid.y != 0:
					routes[direction].push_front(route)
			"up to down":
				route = routes[direction].pop_front()
				
				if route.front().grid.y != dimensions.y - 1:
					routes[direction].push_back(route)
		
		#var key = "up to down"
		#
		#for _i in routes[key].size():
			#route = routes[key][_i]
			#
			#var hue = _i * 1.0 / routes[key].size()
			#
			#for _step in route:
				#_step.rehue(hue)
